...
- If model inspection, switching, or isolated review is unavailable, use the current model, do not claim routing or independence occurred, and disclose the limitation when material.
...
1. Create a contract covering goal, deliverables, constraints, exclusions, acceptance criteria, evidence, dependencies, authorization boundaries, and explicit resource ceilings.
...
4. Before a destructive, irreversible, public, financial, or otherwise consequential mutation, preflight the exact target and payload, authority, recovery or idempotency plan, and single executor. Review it independently when available and verify the resulting state afterward.
5. Run independent lanes in parallel. Use one writer per artifact or isolated workspaces; prevent duplicate actions.
6. Workers execute rather than re-delegate unless authorized. They return `READY_FOR_REVIEW`, `READY_WITH_CONCERNS`, `NEEDS_CONTEXT`, or `WORKER_BLOCKED` with artifacts, evidence, checks, assumptions, and risks. These are nonterminal: only the supervisor assigns task-level `COMPLETE` or `BLOCKED`, and ready means ready for verification rather than accepted.
7. Verify claims against actual artifacts and evidence; a completion message is not proof. Treat `READY_WITH_CONCERNS` as incomplete until the concerns are resolved or genuinely blocked.
8. Independently review material or judgment-heavy work against the contract without inheriting the worker's conclusions.
9. Return defects as focused repairs and re-review affected work. After a failed repair, change the evidence, context, capability, or approach; do not repeat an unchanged attempt. After two repair rounds for the same unresolved defect, the supervisor must reassess before continuing. Begin another round only with new evidence or a materially different viable approach; otherwise return task-level `BLOCKED` without lowering quality.
