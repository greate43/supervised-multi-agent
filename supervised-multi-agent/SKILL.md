...
7. Workers execute rather than re-delegate unless authorized. They return `READY_FOR_REVIEW`, `READY_WITH_CONCERNS`, `NEEDS_CONTEXT`, or `WORKER_BLOCKED` with artifacts, evidence, checks, assumptions, risks, recommended next action, and truthful per-worker usage when available. Reserve `READY_WITH_CONCERNS` for unresolved concerns that could affect a required criterion; report optional observations separately. These statuses are nonterminal: only the supervisor assigns task-level `COMPLETE` or `BLOCKED`, and ready means ready for verification rather than accepted.
8. Verify claims against actual artifacts and evidence; a completion message is not proof. Treat `READY_WITH_CONCERNS` as incomplete until its criterion-affecting concerns are resolved or genuinely blocked.
